...
function Timesheet() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const LC_Email = localStorage.getItem('Email')
            const response = await axios.post(`http://localhost:5000/api/timesheet/storedata/${LC_Email}`, { data: dataToSend });
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:5000/api/resources/getResources/${LC_Email}`);
