...
import './CSS/resourceallocation.css';
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <select id="userId" name="userId"  value={formData.userId} onChange={handleUserSelection} required>
            {users.map(user => (
              <option key={user._id} value={user.firstname}>{user.firstname}</option>
            ))}
          </select>
        </div>
        <div className="form-group">
        ...
        <button type="submit" onClick={handleSubmit}>Allocate Resources</button>
      </form>
    </div>
  );
}
