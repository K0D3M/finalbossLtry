...
const Resources = require('../Model/resources_model');
...
projectController.addProject = async (req, res) => {
  try {
    const { id, name, domain, startDate, endDate, complexity } = req.body;
    const project = new Project({ id, name, domain, startDate, endDate, complexity });
    await project.save();
